...
class CropAdviceService {
  ...
  Future<List<Map<String, dynamic>>> getCropSpecificAdvice(
    ...
    for (final crop in userCrops) {
      ...
      if (plantingMonths.contains(currentMonth)) {
        adviceText +=
            "Planting season for $crop in $region! Prepare your fields and ensure proper soil preparation. ";
      }

      // Check if it's harvesting month
      if (harvestMonths.contains(currentMonth)) {
        adviceText +=
            "Harvesting season for $crop. Monitor maturity and plan harvest timing based on weather conditions. ";
      }
      ...
      if (growingSeason.contains(season)) {
        ...
        if (currentStage.isNotEmpty && growthStages.length > 1) {
          if (currentStage == growthStages.first) {
            adviceText +=
                "Focus on proper establishment and early growth management. ";
          } else if (currentStage == growthStages.last) {
            adviceText +=
                "Prepare for upcoming harvest and monitor maturity indicators. ";
          } else if (currentStage.contains('flower')) {
            adviceText +=
                "Critical flowering period - ensure optimal conditions for good yield. ";
          }
        }
      } else if (growingSeason.isNotEmpty) {
        adviceText +=
            "Current $season season is not ideal for $crop growth. Plan accordingly. ";
      }
      ...
      if (temperature < tempRange['min']) {
        ...
        if (cropData['sensitiveToFrost'] == true) {
          adviceText +=
              "Protect plants from frost damage. Consider using mulch or row covers. ";
        }
      } else if (temperature > tempRange['max']) {
        adviceText += "Temperature is above optimal range for $crop. ";
        if (cropData['sensitiveToHeat'] == true) {
          adviceText +=
              "Provide additional irrigation and avoid midday activities. ";
        }
      } else {
      ...
      if (humidity < humidityRange['min']) {
        adviceText +=
            "Humidity is low for $crop. Consider increasing irrigation frequency. ";
      } else if (humidity > humidityRange['max']) {
        adviceText += "Humidity is high for $crop. ";
        if (condition.contains('Rain') || condition.contains('Drizzle')) {
          adviceText +=
              "Watch for increased disease pressure in wet conditions. ";
        }
      }
      ...
      if (condition.contains('Rain') || condition.contains('Drizzle')) {
        if (cropData['sensitiveToRain'] == true) {
          adviceText +=
              "Current rainy conditions may affect $crop. Ensure proper drainage and monitor for disease. ";
          ...
          if (harvestMonths.contains(currentMonth)) {
            adviceText +=
                "Harvest may be delayed due to rain. Protect harvested produce from moisture. ";
          }
        }
      } else if (condition.contains('Clear') || condition.contains('Sunny')) {
        final String waterRequirement =
            cropData['waterRequirement'] ?? 'moderate';
      ...
      } else if (adviceText.contains("temperature") ||
          adviceText.contains("heat")) {
        iconName = 'thermostat';
        colorHex = '#FFA500'; // Orange
      } else if (adviceText.contains("disease") ||
          adviceText.contains("pest")) {
        iconName = 'bug_report';
        colorHex = '#F44336'; // Red
      } else if (adviceText.contains("harvesting") ||
          adviceText.contains("harvest")) {
        iconName = 'agriculture';
        colorHex = '#8BC34A'; // Light Green
      } else if (adviceText.contains("planting") ||
          adviceText.contains("sowing")) {
        iconName = 'seed';
  ...
  String _estimateGrowthStage(String crop, String currentMonth) {
    ...
    int monthsSincePlanting =
        (currentMonthIndex - closestPlantingMonthIndex) % 12;

    // Estimate growth stage based on months since planting
    int totalGrowthPeriod = 6; // Assume 6 months growth cycle by default
    int stageIndex =
        (monthsSincePlanting * growthStages.length) ~/ totalGrowthPeriod;
